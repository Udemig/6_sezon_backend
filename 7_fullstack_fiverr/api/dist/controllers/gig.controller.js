...
const buildFilters = (query) => {
    ...
    if (query.search)
        filters.title = { $regex: query.search, $options: "i" }; // insensetive (büyük-küçük harf)
    return filters;
};
...
export const getGig = c(async (req, res, next) => {
    const gig = await Gig.findById(req.params.id).populate("user");
    if (!gig)
        return next(e(404, "Aradığınız hizmet bulunamadı"));
    res.status(200).json({
...
export const createGig = c(async (req, res, next) => {
    ...
    if (!req.isSeller)
        return next(e(403, "Sadece satıcı hesapları hizmet oluşturabilir"));
...
export const deleteGig = c(async (req, res, next) => {
    ...
    if (String(gig?.user) !== req.userId)
        return next(e(403, "Bu işlemi yapmaya yetkiniz yok"));
    // hizmeti sil
